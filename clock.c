			  ...
			  struct interface *iface)
{
	...
	/* Redundancy master ports (e.g. hsr0) are not counted as PTP ports
	 * since they only aggregate traffic for the slave ports and do not
	 * participate in the PTP state machine themselves.
	 */
	if (red_master)
...
int clock_poll(struct clock *c)
{
	...
	LIST_FOREACH(p, &c->ports, list) {
		...
		for (i = 0; i < N_POLLFD; i++) {
			...
			/* Before we move onto the next fd, restore the
			 * port being iterated.
			 */
			if (piter) {
				port_release_dispatch_port(piter, p);
...
static void handle_state_decision_event(struct clock *c)
{
	...
	LIST_FOREACH(piter, &c->ports, list) {
		...
		case PS_MASTER:
			...
			break;
		case PS_PASSIVE:
			pr_notice("%s assuming the PASSIVE role",
				  port_name(piter));
			event = EV_RS_PASSIVE;
			break;
		case PS_SLAVE:
			pr_notice("%s assuming the SLAVE role",
				  port_name(piter));
			clock_update_slave(c);
			event = EV_RS_SLAVE;
			break;
		case PS_PASSIVE_SLAVE:
			pr_notice("%s assuming the PASSIVE_SLAVE role",
				  port_name(piter));
			event = EV_RS_PASSIVE_SLAVE;
			break;
		default:
			event = EV_FAULT_DETECTED;
			break;
