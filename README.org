...
* Microchip changes
  ...
  We have tried to extend the existing set of features to support also
  IEEE 802.1AS-2020 in the role of relay. We will try to send for
  upstreaming these patches and if they will be accepted then we will stop
  using this repo and start to use original one.

  Currently is still missing the redundant support.
